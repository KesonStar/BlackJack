...
class BlackJack:
    ...
    suits = {"diamonds": diamond, "hearts": heart, "spades": spade, "clubs": club}
    ...
    def generate_deck(self):
        ...
        deck = [
            {"number": number, "suit": suit} for number in numbers for suit in suits
        ]
        return deck
    ...
    def play(self, player_action: str, dealer_strategy: str, output=False):
        ...
        if output:
            print(
                ...
                game.total_value(game.dealer_hand),
            )
        return self.status
...
if __name__ == "__main__":
    ...
    for round in range(5):
        ...
        while game.status == "continue":
            print(
                game.format_cards(game.player_hand), game.total_value(game.player_hand)
            )
            action = input("Enter an action (hit/stay): ")
            game.play(action, "basic")
        print(
            f"dealer's hand: {game.format_cards(game.dealer_hand)} {game.get_dealervalue()}  player's hand: {game.format_cards(game.player_hand)} {game.get_playervalue()}"
        )
